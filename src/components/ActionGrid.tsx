          ...
          <div className={`w-full bg-white rounded-xl border-2 ${activeTourStep === 'services' ? 'border-blue-300 shadow-lg' : 'border-slate-100 shadow-sm'} p-2 transition-all duration-300`}>
            <div className="grid grid-cols-4 gap-1.5">
              {services && services.length > 0 ? (
                services.filter((s: any) => s.is_active !== false).sort((a: any, b: any) => (a.display_order || 0) - (b.display_order || 0)).map((svc: any) => {
                  if (svc.icon_name?.toLowerCase() === 'talabat') {
                    return (
                      <a key={svc.id} href={s?.talabat || s?.social_links?.talabat || "https://www.talabat.com"} target="_blank" onClick={() => trackAction('talabat_click')} onTouchStart={() => {}} className="flex flex-col items-center justify-center bg-transparent rounded-xl p-1.5 group/btn transition-all h-[72px] cursor-pointer hover:bg-[#FF5A00]/15 active:bg-[#FF5A00]/15 border-2 border-transparent hover:border-[#FF5A00] active:border-[#FF5A00]">
                        ...
                        <span className="text-[9px] font-bold text-slate-500 whitespace-nowrap mt-0.5 group-hover/btn:text-white group-active/btn:text-white transition-colors">{svc.title}</span>
                      </a>
                    );
                  }
                  ...
                  return (
                    <button key={svc.id} onTouchStart={() => {}} className={cell()}>
                      ...
                      <span className={svcLabel}>{svc.title}</span>
                    </button>
                  );
                })
              ) : (
                <>
                  <button onTouchStart={() => {}} className={cell()}><Icons.Activity size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>استشارة</span></button>
                  <button onTouchStart={() => {}} className={cell()}><Icons.Search size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>توفير نواقص</span></button>
                  <button onTouchStart={() => {}} className={cell()}><Icons.HeartPulse size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>سكر دم</span></button>
                  <button onTouchStart={() => {}} className={cell()}><Icons.Activity size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>قياس ضغط</span></button>
                  <button onTouchStart={() => {}} className={cell()}><Icons.Stethoscope size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>InBody</span></button>
                  <a href={s?.talabat || s?.social_links?.talabat || "https://www.talabat.com"} target="_blank" onClick={() => trackAction('talabat_click')} onTouchStart={() => {}} className="flex flex-col items-center justify-center bg-transparent rounded-xl p-1.5 group/btn transition-all h-[72px] cursor-pointer hover:bg-[#FF5A00]/15 active:bg-[#FF5A00]/15 border-2 border-transparent hover:border-[#FF5A00] active:border-[#FF5A00]">
                    <div className="w-9 h-9 flex items-center justify-center mb-0.5 group-hover/btn:scale-110 group-active/btn:scale-110 transition-transform"><div className="bg-[#FF5A00] text-white rounded-md w-10 h-10 flex items-center justify-center font-black text-[10px] italic">talabat</div></div>
                  ...
                  <button onTouchStart={() => {}} className={cell()}><Icons.Bike size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>توصيل</span></button>
                  <button onTouchStart={() => {}} className={cell()}><Icons.CreditCard size={27} strokeWidth={2} className={svcIcon} /><span className={svcLabel}>Visa</span></button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="relative mt-0.5 mb-1">
          ...
          <div className={`w-full bg-white rounded-xl border-2 ${activeTourStep === 'socials' ? 'border-[#5D9CFF] shadow-lg' : 'border-slate-100 shadow-sm'} p-2 pt-3 transition-all duration-300`}>
            <div className="grid grid-cols-3 gap-1.5 mb-1.5">
              <a href={`tel:${s?.social_links?.phone || '01000000000'}`} onClick={() => trackAction('call_mobile')} onTouchStart={() => {}} className="flex flex-row-reverse items-center justify-center gap-1.5 bg-transparent rounded-xl p-2 group/subbtn hover:border-blue-500 active:border-blue-500 hover:bg-blue-500/10 active:bg-blue-500/10 transition-all h-12 bg-white border-2 border-slate-100">
              ...
              <a href={`https://wa.me/${s?.whatsapp_number || '201000000000'}`} target="_blank" onClick={() => trackAction('whatsapp_click')} onTouchStart={() => {}} className="flex flex-row-reverse items-center justify-center gap-1.5 bg-transparent rounded-xl p-2 group/subbtn hover:border-[#25D366] active:border-[#25D366] hover:bg-[#25D366]/10 active:bg-[#25D366]/10 transition-all h-12 bg-white border-2 border-slate-100">
              ...
              <a href={`tel:${s?.social_links?.landline || '0220000000'}`} onClick={() => trackAction('call_landline')} onTouchStart={() => {}} className="flex flex-row-reverse items-center justify-center gap-1.5 bg-transparent rounded-xl p-2 group/subbtn hover:border-slate-400 active:border-slate-400 hover:bg-slate-500/10 active:bg-slate-500/10 transition-all h-12 bg-white border-2 border-slate-100">
            ...
            <div className="grid grid-cols-4 gap-1.5">
              <a href={s?.social_links?.facebook || 'https://facebook.com'} target="_blank" onClick={() => trackAction('facebook_click')} onTouchStart={() => {}} className="flex flex-col items-center justify-center gap-1 bg-transparent rounded-xl p-1 group/btn hover:border-[#1877F2] active:border-[#1877F2] hover:bg-[#1877F2]/10 active:bg-[#1877F2]/10 transition-all h-[58px] bg-white border-2 border-slate-100">
              ...
              <a href={s?.social_links?.instagram || 'https://instagram.com'} target="_blank" onClick={() => trackAction('instagram_click')} onTouchStart={() => {}} className="flex flex-col items-center justify-center gap-1 bg-transparent rounded-xl p-1 group/btn hover:border-[#cc2366] active:border-[#cc2366] hover:bg-[#cc2366]/10 active:bg-[#cc2366]/10 transition-all h-[58px] bg-white border-2 border-slate-100">
              ...
              <a href={s?.social_links?.location_url || 'https://maps.google.com'} target="_blank" onClick={() => trackAction('location_click')} onTouchStart={() => {}} className="flex flex-col items-center justify-center gap-1 bg-transparent rounded-xl p-1 group/btn hover:border-[#EA4335] active:border-[#EA4335] hover:bg-[#EA4335]/10 active:bg-[#EA4335]/10 transition-all h-[58px] bg-white border-2 border-slate-100">
